...
export class AceEditor implements IEditor {
    ...
    onTokenClicked = new EventEmitter<IToken>();
    ...
    private addListener() {
        let el = $(`#${this.embeddedId}`);
        el.on("click", this.onDocumentClick);
    }

    private removeListener(){
        let el = $(`#${this.embeddedId}`);
        el.off("click", this.onDocumentClick);
    }
    ...
    getElement(): any {
        let result = $("body").find(this.embeddeSelector);
        return result;
    }
    ...
    getOrCreateElement(parent: string | any): Promise<any> {
        return new Promise(resolve=>{
            let parentEl = $(parent);
            let result = parentEl.find(this.embeddeSelector);
            if (result.length > 0) {
                resolve(result[0]);
                return;
            }
            let embedded = $(`<div id="${this.embeddedId}" style="${embeddedStyle}"></div>`)
            parentEl.append(embedded);
            setTimeout(() => { resolve(embedded[0]) });
        });
    }
    ...
    async attach(parent: string | any): Promise<void> {
        let element = await this.getOrCreateElement(parent);
        return new Promise((resolve) => {
            if (this.aced) {
                $(element).show();
                this.addListener();
